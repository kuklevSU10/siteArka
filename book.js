...
(function () {
  ...
  function buildPagesHTML(projects) {
    ...
    html += page("book-page-paper book-create-page", "", runner("АРКА \\\\ Каталог", "Что мы создаём") +
      '<div class="book-create-wrap">' +
        '<p class="book-intro-k">Что мы создаём</p>' +
        '<ul class="book-create">' +
          '<li><h4>Частные интерьеры</h4><p>Функциональные интерьеры для жизни.</p></li>' +
          '<li><h4>Коммерческие пространства</h4><p>Пространства, которые работают на бизнес.</p></li>' +
          '<li><h4>Проекты для девелоперов</h4><p>Дизайн-решения для мест общего пользования и паркинга.</p></li>' +
        '</ul>' +
      '</div>' +
      folio(nextFolio()));
    var toc = "";
